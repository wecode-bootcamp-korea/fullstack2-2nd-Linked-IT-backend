...
const InsertData = (fileName) => {
  ...
  fs.createReadStream(filePath)
    ...
    .on('data', async (data) => {
      ...
      // if (fileName === 'scope_of_publics')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //   scope_of_publics (id,type)
      //   VALUES
      //   (${data.id}, ${data.type});`;
      if (fileName === 'educations')
        await prisma.$queryRaw`
          INSERT INTO
            educations (id,admission_month,admission_year,graduation_month,graduation_year,grade,activity,description,college_id,degree_id,major_id,user_id,scope_of_public_id)
          VALUES
          (${data.id}, ${data.admission_month}, ${data.admission_year}, ${data.graduation_month}, ${data.graduation_year}, ${data.grade}, ${data.activity}, ${data.description}, ${data.college_id}, ${data.degree_id}, ${data.major_id}, ${data.user_id}, ${data.scope_of_public_id});`;
      // if (fileName === 'positions')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      ...
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     contact_informations (id,profile_url,phone_number,phone_type,address,birth_year,birth_month,birth_day,user_id,scope_of_public_id)
      //   VALUES
      //     (${data.id}, ${data.profile_url}, ${data.phone_number}, ${data.phone_type}, ${data.address}, ${data.birth_year}, ${data.birth_month}, ${data.birth_day}, ${data.user_id}, ${data.scope_of_public_id});`;
      ...
      //   await prisma.$queryRaw`
      //     INSERT INTO
      //       position_careers (id,is_current_position,is_end_current_position,headline,description,position_id,company_id,industry_id,user_id,employment_type_id,end_month,end_year,start_month,start_year,scope_of_public_id)
      //     VALUES
      //     (${data.id}, ${data.is_current_position}, ${data.is_end_current_position}, ${data.headline}, ${data.description}, ${data.position_id}, ${data.company_id}, ${data.industry_id}, ${data.user_id}, ${data.employment_type_id}, ${data.end_month}, ${data.end_year}, ${data.start_month}, ${data.start_year}, ${data.scope_of_public_id});`;
      ...
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     friends (user_id,friend_id,friend_status_id)
      //   VALUES
      //     (${data.user_id}, ${data.friend_id}, ${data.friend_status_id});`;
      // if (fileName === 'post_attachments')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     post_attachments (id, image_url, media)
      //   VALUES
      //     (${data.id}, ${data.image_url}, ${data.media});`;
      // if (fileName === 'posts')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     posts (id, content, created_at, updated_at, deleted_at, like_count, post_scope_of_public_id, user_id, post_attachment_id, comment_scope_of_public_id)
      //   VALUES
      //     (${data.id}, ${data.content}, ${data.created_at}, ${data.updated_at}, ${data.deleted_at}, ${data.like_count}, ${data.post_scope_of_public_id}, ${data.user_id},  ${data.post_attachment_id}, ${data.comment_scope_of_public_id});`;
      // if (fileName === 'comments')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     comments (id, content, created_at, updated_at, deleted_at, is_deleted, order,	like_count, comment_id, user_id, post_id)
      //   VALUES
      //     (${data.id}, ${data.content}, ${data.created_at}, ${data.updated_at}, ${data.deleted_at}, ${data.is_deleted}, ${data.order}, ${data.like_count}, ${data.comment_id}, ${data.user_id}, ${data.post_id});`;
      // if (fileName === 'post_like_styles')
      //   await prisma.$queryRaw`
      //   INSERT INTO
      //     post_like_styles (id, type)
      //   VALUES
      // (${data.id}, ${data.type});`;
      try {
        csvData.push(data);
